...
// @version         0.0.4
// @Note           [0.0.2.170217]Nightly54 2017.02.16 兼容性修复。
// @Note           [0.0.3.170424]Nightly55 修复移除 browser.tabs.animate 参数后，中键新标签打开失效的问题。
// @Note           [0.0.4.170527]修复和简化继承历史的实现
// ==/UserScript==
...
location == 'chrome://browser/content/browser.xul' && 'ContextualIdentityService' in window && (function () {
	let {classes: Cc, interfaces: Ci} = Components,
		...
		getUserContextLabel = CIS.getUserContextLabel.bind(CIS),
		getIdentities = (CIS.getPublicIdentities || CIS.getIdentities).bind(CIS),
		getIdentityFromId = (CIS.getPublicIdentityFromId || CIS.getIdentityFromId).bind(CIS);
	...
	let onPopupshowing = event => {
		...
		getIdentities().forEach(identity => {
			if (identity.userContextId == 0)
				return;
			let menuitem = document.createElement('menuitem');
			menuitem.setAttribute('data-usercontextid', identity.userContextId);
			menuitem.setAttribute('label', getUserContextLabel(identity.userContextId));
	...
	updateContainerMenu = id => {
		...
		if (!id) {
			...
			let identity = getIdentityFromId(id);
			menu.setAttribute('label', `${text2}: "${getUserContextLabel(identity.userContextId)}"`);
	...
	let toggleContainerTab = (id, isDuplicate = false, url) => {
		...
		//还原历史会话
		let oldState = JSON.parse(ss.getTabState(tab));
		let newState = oldState;
		if(url){
			//打开书签覆盖当前页继承历史
			newState = JSON.parse(ss.getTabState(t));
			Array.prototype.unshift.apply(newState.entries, oldState.entries);
			newState.index += oldState.entries.length;
		}
		newState.userContextId = id;
		ss.setTabState(t, JSON.stringify(newState));
